...
export function PremiumClient() {
  ...
  const FEATURES = [
    {
      icon: Star,
      title: t("features.highlightTitle"),
      description: t("features.highlightDesc"),
    },
    {
      icon: BarChart3,
    ...
    {
      icon: TrendingUp,
      title: t("features.fairPriceTitle"),
      description: t("features.fairPriceDesc"),
    },
  ];
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error || t("form.somethingWentWrong"));
      } else {
        setSuccess(true);
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-lg font-bold">{t("tiers.legacyTitle")}</h3>
          </div>
          <p className="text-2xl font-bold text-amber-400">{t("tiers.legacyPrice")}</p>
            ...
            <div className={`mt-2 inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-medium ${
              ...
              {legacyCount.isFull ? (
                <>
                  ...
                  {t("tiers.soldOut", { limit: legacyCount.limit })}
                </>
              ) : (
                <>
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-lg font-bold">{t("tiers.subscriberTitle")}</h3>
          </div>
          <p className="text-2xl font-bold text-primary">{t("tiers.subscriberPrice")}</p>
          <p className="text-sm text-muted-foreground">
            {t("tiers.subscriberDesc")}
          </p>
        </div>
      </div>
      ...
      {status === "loading" ? (
        ...
        <div className="rounded-xl border border-amber-400/30 bg-amber-400/5 p-8 text-center space-y-2">
          ...
          <p className="text-lg font-semibold text-amber-400">
            {t("activeStatus", { tier: session.user.premiumTier === "legacy" ? t("tiers.legacyTitle") : t("tiers.subscriberTitle") })}
          </p>
          <p className="text-sm text-muted-foreground">
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-xl border border-border bg-card p-6 space-y-3">
            <h3 className="font-semibold">{t("instructions.heading")}</h3>
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  ...
                  {pendingRequest.requestedTier === "legacy"
                    ? t("requestHistory.legacy")
                    : t("requestHistory.subscriber")}
