...
function VocationSpeedHint({ vocation, category }: { vocation: Vocation; category: SkillCategory }) {
  ...
  let dotColor: string;
  if (b <= 1.1) {
    speed = t('speedHints.primary');
    dotColor = 'bg-emerald-400';
  } else if (b <= 1.2) {
    speed = t('speedHints.secondary');
    dotColor = 'bg-green-400';
  } else if (b <= 1.5) {
    speed = t('speedHints.moderate');
    dotColor = 'bg-yellow-400';
  } else if (b <= 2.0) {
    speed = t('speedHints.slow');
    dotColor = 'bg-orange-400';
  } else {
    speed = t('speedHints.verySlow');
    dotColor = 'bg-red-400';
  }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-3 overflow-hidden rounded-2xl" style={{ background: 'rgba(21,28,42,0.6)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)', border: '1px solid rgba(255,255,255,0.06)' }}>
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <StyledInput
                label={t('labelCurrentSkillLevel')}
                value={currentSkill}
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                {[
                  { checked: privateDummy, set: setPrivateDummy, icon: <Shield className="w-4 h-4 text-blue-400" />, label: t('modifiers.privateDummy'), desc: t('modifiers.privateDummyDesc') },
                  { checked: doubleEvent, set: setDoubleEvent, icon: <Zap className="w-4 h-4 text-yellow-400" />, label: t('modifiers.doubleEvent'), desc: t('modifiers.doubleEventDesc') },
                  { checked: vip, set: setVip, icon: <Crown className="w-4 h-4 text-amber-400" />, label: t('modifiers.vipAccount'), desc: t('modifiers.vipAccountDesc') },
                ].map((mod) => (
                  <label
                    key={mod.label}
            ...
            {mode === 'target' && weaponsResult && (
              <div className="space-y-5">
                      ...
                      >
                        <div>
                          ...
                          <div className="text-xs text-muted-foreground/50">
                            {formatNumber(WEAPON_TYPES[i].charges)} {t('chargesEach')}
                          </div>
                        </div>
                ...
                <div className="flex items-center gap-3 p-3.5 bg-background/30 rounded-lg border border-border/15">
                  ...
                  <div>
                    <div className="text-xs text-muted-foreground/50 uppercase tracking-wide font-semibold">{t('results.estimatedTime')}</div>
                ...
                <div className="border-t border-border/15 pt-4 space-y-1.5">
                  {[
                    [t('results.vocation'), VOCATIONS.find(v => v.value === vocation)?.label],
                    [t('results.from'), `Skill ${current} (${pctToGo.toFixed(2)}% to go)`],
                    [t('results.to'), `Skill ${target}`],
                    [t('results.serverMultiplier'), `${getMultiplierForDisplay(current, category)}x`],
                    [t('results.vocationRate'), `b=${getVocationConstant(vocation, category)}`],
                  ].map(([k, v]) => (
            ...
            {mode === 'weapons' && skillResult && (
              <div className="space-y-5">
                ...
                <div className="border-t border-border/15 pt-4 space-y-1.5">
                  {[
                    [t('results.vocation'), VOCATIONS.find(v => v.value === vocation)?.label],
                    [t('results.weapon'), `${count}x ${WEAPON_TYPES[weaponType].name}`],
                    [t('results.totalCharges'), formatNumber(WEAPON_TYPES[weaponType].charges * count)],
                    [t('results.pricePerWeapon'), `${WEAPON_TYPES[weaponType].rcCost} RC`],
                    [t('results.serverMultiplier'), `${getMultiplierForDisplay(current, category)}x`],
                    [t('results.vocationRate'), `b=${getVocationConstant(vocation, category)}`],
                  ].map(([k, v]) => (
                    <div key={k} className="flex justify-between text-sm">
            ...
            {((mode === 'target' && !weaponsResult) || (mode === 'weapons' && !skillResult)) && (
              <div className="text-center py-16">
                ...
                <p className="text-base text-muted-foreground/50 max-w-[200px] mx-auto">
                  {mode === 'target'
                    ? t('placeholderWeapons')
                    : t('placeholderSkillGain')}
                </p>
              </div>
            )}
          </div>
        ...
        <div className="p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-base font-bold">
              {t('multiplierTable.heading', { skill: SKILL_CATEGORIES.find(c => c.value === category)?.label ?? '' })}
            </h2>
          </div>
