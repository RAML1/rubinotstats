...
function getReasonColor(reason: string | null): string {
  ...
  // Cheating / software / automation
  if (r.includes('illegal') || r.includes('automation') || r.includes('multi-client') || r.includes('bot')) return 'text-red-400';
  // Names
  if (r.includes('name')) return 'text-yellow-400';
  // Abuse / exploits
  if (r.includes('abuse') || r.includes('bug') || r.includes('ks') || r.includes('lure')) return 'text-orange-400';
  // Conduct / offensive / harassment
  if (r.includes('offensive') || r.includes('conduct') || r.includes('slander')) return 'text-rose-400';
  // Fraud / transactions
  if (r.includes('fraud') || r.includes('bazaar') || r.includes('market')) return 'text-purple-400';
  // Advertising / spam
  if (r.includes('advertising') || r.includes('spam')) return 'text-cyan-400';
  // Staff impersonation / attacks
  if (r.includes('impersonat') || r.includes('attack') || r.includes('law')) return 'text-red-500';
...
export function BansClient({ initialBans, initialTotal, insights }: BansClientProps) {
  ...
  const pieData = [
    { name: t('charts.permanent'), value: insights.permanent },
    { name: t('charts.temporary'), value: insights.temporary },
  ];
        ...
        <div className="overflow-hidden rounded-2xl" style={{ background: 'rgba(21,28,42,0.6)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)', border: '1px solid rgba(255,255,255,0.06)' }}>
          <div className="p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="text-sm font-semibold">{t('charts.bansByRule')}</h3>
            </div>
        ...
        <div className="overflow-hidden rounded-2xl" style={{ background: 'rgba(21,28,42,0.6)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)', border: '1px solid rgba(255,255,255,0.06)' }}>
          <div className="p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="text-sm font-semibold">{t('charts.banDuration')}</h3>
            </div>
              ...
              <div className="space-y-3 flex-1">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">{t('charts.permanent')}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">{t('charts.temporary')}</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="overflow-hidden rounded-2xl" style={{ background: 'rgba(21,28,42,0.6)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)', border: '1px solid rgba(255,255,255,0.06)' }}>
          <div className="p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="text-sm font-semibold">{t('charts.bansOverTime')}</h3>
            </div>
            <ResponsiveContainer width="100%" height={160}>
              <BarChart
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          >
            ...
            <span className="text-sm font-bold">{t('activeBans', { count: total.toLocaleString() })}</span>
          </div>
        </div>
          ...
          <table className="w-full text-[13px]">
            <thead>
              <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.06)', background: 'rgba(255,255,255,0.02)' }}>
                <th className="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider" style={{ color: 'rgba(236,240,247,0.38)' }}>{t('table.player')}</th>
                <th className="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider" style={{ color: 'rgba(236,240,247,0.38)' }}>{t('table.reason')}</th>
                <th className="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider" style={{ color: 'rgba(236,240,247,0.38)' }}>{t('table.banned')}</th>
                <th className="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider" style={{ color: 'rgba(236,240,247,0.38)' }}>{t('table.expires')}</th>
              </tr>
            </thead>
              ...
              {filteredBans.length === 0 ? (
                <tr>
                  <td colSpan={4} className="px-4 py-12 text-center text-muted-foreground">
                    {search ? t('table.emptySearch') : t('table.emptyNoData')}
                  </td>
                </tr>
              ) : (
