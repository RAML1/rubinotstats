...
function FlowLegend({ arrivalsLabel, departuresLabel }: { arrivalsLabel: string; departuresLabel: string }) {
  return (
    <div className="flex items-center justify-center gap-4 mt-1">
      <div className="flex items-center gap-1.5">
        ...
        <span className="text-[10px] text-muted-foreground">{arrivalsLabel}</span>
      </div>
      <div className="flex items-center gap-1.5">
        <div className="h-2.5 w-2.5 rounded-sm" style={{ background: 'linear-gradient(180deg, #FB7185, rgba(251,113,133,0.3))' }} />
        <span className="text-[10px] text-muted-foreground">{departuresLabel}</span>
      </div>
    </div>
  );
}
...
export function TransfersClient({ initialTransfers, initialTotal, worlds, insights }: TransfersClientProps) {
  ...
  async function fetchPage(newPage: number, opts?: { search?: string; from?: string; to?: string }) {
    ...
    try {
      ...
      const tw = opts?.to ?? toWorld;
      if (s) params.set('search', s);
      if (f) params.set('fromWorld', f);
      if (tw) params.set('toWorld', tw);
        ...
        <Card className="border-border/50 bg-card/50 overflow-hidden md:col-span-2">
          <CardContent className="p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="text-sm font-semibold">{t('charts.worldPopulationFlow')}</h3>
            </div>
            <ResponsiveContainer width="100%" height={180}>
              ...
              >
                ...
                <Bar dataKey="arrivals" name={t('charts.arrivals')} fill="url(#arrivalGrad)" radius={[6, 6, 0, 0]} />
                <Bar dataKey="departures" name={t('charts.departures')} fill="url(#departGrad)" radius={[6, 6, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
            <FlowLegend arrivalsLabel={t('charts.arrivals')} departuresLabel={t('charts.departures')} />
          </CardContent>
        </Card>
        ...
        <Card className="border-border/50 bg-card/50 overflow-hidden">
          <CardContent className="p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="text-sm font-semibold">{t('charts.transferLevels')}</h3>
            </div>
            <ResponsiveContainer width="100%" height={180}>
              <BarChart
        ...
        <Card className="border-border/50 bg-card/50 overflow-hidden">
          <CardContent className="p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="text-sm font-semibold">{t('charts.mostPopularRoutes')}</h3>
            </div>
      ...
      <div className="flex flex-col gap-3">
        <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 rounded-lg bg-blue-500/10 border border-blue-500/20 px-3 py-1.5">
              ...
              <span className="text-sm font-medium text-blue-300">{t('transfersCount', { count: total.toLocaleString() })}</span>
            </div>
          </div>
        ...
        <div className="flex flex-wrap gap-2">
          <div className="flex items-center gap-1.5">
            ...
            >
              <option value="">{t('fromAllWorlds')}</option>
          ...
          <div className="flex items-center gap-1.5">
            ...
            <select
              ...
              className="h-8 rounded-md border border-border/50 bg-card/50 px-2 text-xs outline-none focus:border-primary/50"
            >
              <option value="">{t('toAllWorlds')}</option>
              {worlds.map(w => <option key={w} value={w}>{w}</option>)}
            </select>
          </div>
          {(fromWorld || toWorld) && (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border/50 bg-muted/30">
                  <th className="px-4 py-3 text-left font-medium text-muted-foreground">{t('table.player')}</th>
                  <th className="px-4 py-3 text-left font-medium text-muted-foreground">{t('table.level')}</th>
                  <th className="px-4 py-3 text-left font-medium text-muted-foreground">{t('table.from')}</th>
                  <th className="px-4 py-3 text-center font-medium text-muted-foreground w-8"></th>
                  <th className="px-4 py-3 text-left font-medium text-muted-foreground">{t('table.to')}</th>
              ...
              <tbody className={loading ? 'opacity-50' : ''}>
                {filteredTransfers.length === 0 ? (
                  ...
                  filteredTransfers.map((entry) => (
                    ...
                    >
                      <td className="px-4 py-2.5 font-medium">{entry.playerName}</td>
